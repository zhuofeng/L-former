...
from models.Stylegan_networks import SynthesisBlock
...
class WindowAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        """
        B_, N, C = q.shape # 4,16,256
        
        q = q.reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) # 把channel 分成n个head [4, 8, 16, 32]
        ...
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1)) # [4, 8, 16, 16])
...
class StyleSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop) # residual?
    ...
    def forward(self, x, style):
        ...
        qkv = self.qkv(x).reshape(B, -1, 3, C).permute(2, 0, 1, 3).reshape(3 * B, H, W, C) # ([12, 4, 4, 512]) qkv is a linear layer
        qkv_1 = qkv[:, :, :, : C // 2].reshape(3, B, H, W, C // 2) # ([3, 4, 4, 4, 256]) nothing about swin ath here
        
        if self.shift_size > 0: # swin?
            qkv_2 = torch.roll(qkv[:, :, :, C // 2:], shifts=(-self.shift_size, -self.shift_size), dims=(1, 2)).reshape(3, B, H, W, C // 2) # [3, 4, 4, 4, 256]
        else:
            qkv_2 = qkv[:, :, :, C // 2:].reshape(3, B, H, W, C // 2)
        
        q1_windows, k1_windows, v1_windows = self.get_window_qkv(qkv_1) # also calculates attention based on windows # # [4, 16, 256]
        ...
        x = torch.cat([x1.reshape(B, H * W, C // 2), x2.reshape(B, H * W, C // 2)], dim=2) # Here is dobule attention's concation
...
class Generator_new(nn.Module):
    def __init__(
        ...
        self.stylelayers = nn.ModuleList()
        self.to_rgbs = nn.ModuleList()
        num_layers = 0
        
        for i_layer in range(start, end):
            ...
            if i_layer <= 4:
                ...
                in_channel = in_channels[i_layer - start]
                out_channels = in_channels[i_layer - start + 1]
                is_last = (i_layer == end-1)
                layer = SynthesisBlock(in_channel, out_channels, w_dim=self.style_dim, resolution=2**(i_layer+1),
                    img_channels=3, is_last=is_last, use_fp16=True)
                self.stylelayers.append(layer)
                num_layers += 2
    ...
    def forward(
        ...
        x = self.input(latent) # the input layer. latent: ([4, 14, 512]), x: ([4, 512, 4, 4])
        ...
        # some operations to ws

        block_ws = []
        w_idx = int(len(self.swinlayers) * 2 - 1)
        
        for i in range(len(self.stylelayers)):
            block = self.stylelayers[i]
            block_ws.append(latent.narrow(1, w_idx, block.num_conv + block.num_torgb))
            w_idx += block.num_conv
        
        # layers from styleswin
        for layer, to_rgb in zip(self.swinlayers, self.to_rgbs):
            x = layer(x, latent[:,count,:], latent[:,count+1,:]) # after first operation: [4, 64, 512] need some modification at here to modify
            b, n, c = x.shape
            h, w = int(math.sqrt(n)), int(math.sqrt(n))
            skip = to_rgb(x.transpose(-1, -2).reshape(b, c, h, w), skip) # here made the layer and skip into two (styleblock and toRGB into two)
            count = count + 2
        
        b, n, c = x.shape
        h, w = int(math.sqrt(n)), int(math.sqrt(n))
        synthesiscount = 0
        # layers from stylegan
        for layer, cur_ws in zip(self.stylelayers, block_ws):
            if synthesiscount == 0:
                x, skip = layer(x.transpose(-1, -2).reshape(b, c, h, w), skip, cur_ws)
            else:
                x, skip = layer(x, skip, cur_ws)
            synthesiscount = synthesiscount + 1
            count = count + 1
        
        B, C, h, w = x.shape
        assert h * w == self.size * self.size
        image = skip
        if return_latents:
            return image, latent
        else:
