...
class StyleSwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, style):
        ...
        qkv = self.qkv(x).reshape(B, -1, 3, C).permute(2, 0, 1, 3).reshape(3 * B, H, W, C) # ([12, 4, 4, 512])
        qkv_1 = qkv[:, :, :, : C // 2].reshape(3, B, H, W, C // 2) # ([3, 4, 4, 4, 256])
        ...
        q1_windows, k1_windows, v1_windows = self.get_window_qkv(qkv_1) # dobule attention?
...
class Generator_stylegan(nn.Module):
    def __init__(
        self,
        size,
        style_dim,
        ...
        self.block_resolutions = [2 ** i for i in range(2, self.end + 1)]
        channel_base = 32768
        channel_max = 512
        ...
        for i in range(len(self.block_resolutions)):
            ...
            is_last = (res == self.size)
            layer = SynthesisBlock(in_channels, out_channels, w_dim=self.style_dim, resolution=res,
                img_channels=3, is_last=is_last, use_fp16=True)
            self.stylelayers.append(layer)
            self.n_latent += layer.num_conv
            if is_last:
                self.n_latent += layer.num_torgb
            setattr(self, f'b{res}', layer)
    ...
    def flops(self):
        flops = 0
        for _, layer in enumerate(self.stylelayers):
            flops += layer.flops()
        # 8 FC + PixelNorm
