...
class StyleSwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, style):
        ...
        qkv = self.qkv(x).reshape(B, -1, 3, C).permute(2, 0, 1, 3).reshape(3 * B, H, W, C) # ([12, 4, 4, 512]) just linear
        qkv_1 = qkv[:, :, :, : C // 2].reshape(3, B, H, W, C // 2) # ([3, 4, 4, 4, 256])
        
        if self.shift_size > 0: # swin? if swin, roll the feature vector
        ...
        x1 = self.attn[0](q1_windows, k1_windows, v1_windows, self.attn_mask1) # this does not swin
        x2 = self.attn[1](q2_windows, k2_windows, v2_windows, self.attn_mask2) # this does swin, infact combined swin-transformer insider it
        ...
        x = self.proj(x) # this is the projection after 
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        for layer, to_rgb in zip(self.layers, self.to_rgbs):
            x = layer(x, latent[:,count,:], latent[:,count+1,:]) # x after first operation: [4, 64, 512]
            b, n, c = x.shape
